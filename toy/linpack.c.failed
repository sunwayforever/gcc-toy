...
int main() {
    ...
    /* fprintf(stderr, PREC); */
    /* fprintf(stderr, "Precision Linpack Benchmark - PC Version in
     * 'C/C++'\n\n"); */
    ...
    assert(fabs(residn - 1.596) < 0.001);
    ...
    if (job == 0) {
        ...
        if (nm1 >= 1) {
            for (kb = 1; kb < nm1; kb++) {
                k = n - (kb + 1);
                b[k] = b[k] +
                       ddot(n - (k + 1), &a[lda * k + k + 1], 1, &b[k + 1], 1);
                l = ipvt[k];
                if (l != k) {
                    t = b[l];
                    b[l] = b[k];
...

{
    ...
    for (i = 0; i < n; i++) dtemp = dtemp + dx[i] * dy[i];
...

{
    ...
    if (incx != 1) {
        ...
        nincx = n * incx;
        for (i = 0; i < nincx; i = i + incx) dx[i] = da * dx[i];
...
*/
{
    ...
    if (j >= 1) {
        j = j - 1;
        for (i = 0; i < n1; i++) y[i] = (y[i]) + x[j] * m[ldm * j + i];
    }
